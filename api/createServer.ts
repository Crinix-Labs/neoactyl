...
async function getRandomAllocation(node: string): Promise<number> {
    try {
        const response = await axios.get(`${config.pterodactyl.panel}/api/application/nodes/${node}/allocations`, {
            headers: {
        ...
        const allocations = response.data.data.filter((allocation: any) => !allocation.attributes.assigned);
...
router.post("/api/server", async (req, res) => {
    const { name, egg_id, username, ram, disk, cpu, allocation, database, node, backups } = req.body;
    ...
    try {
        ...
        if (usert.servers >= usert.slots) {
            return res.status(400).json({ status: "failed", message: "Server Limit reached!" });
        }
        if (usert.ram < ram) {
            return res.status(400).json({ status: "failed", message: "Insufficient RAM available" });
        }
        if (usert.disk < disk) {
            return res.status(400).json({ status: "failed", message: "Insufficient disk space available" });
        }
        if (usert.cpu < cpu) {
            return res.status(400).json({ status: "failed", message: "Insufficient CPU available" });
        }
        ...
        if (!egg) {
            return res.status(400).json({ status: "failed", message: "Invalid Egg ID" });
        }
        if (!egg.enabled) {
            return res.status(403).json({ status: "declined", message: "This egg is disabled" });
        }
        ...
        await axios.post(`${config.pterodactyl.panel}/api/application/servers`, body, {
            headers: {
                Authorization: `Bearer ${config.pterodactyl.api}`,
                "Content-Type": "application/json",
                Accept: "application/json",
